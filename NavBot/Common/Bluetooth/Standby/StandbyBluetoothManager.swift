...
class StandbyBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    
    var current_isStandByStatuse = false
    var will_isStandByStatuse = false
    
    //MARK: 1.初始化-单例
    static let shared = StandbyBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "standbyDevice"){
            //print("SetTimePudcam--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "standbyDevice_fail"), object: nil)
        }else{
            //现在等待返回数据--如果10秒内没有返回数据，就认为失败了
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("standbyDevice_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "standbyDevice"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("standbyDevice--收到的数据:\(allReceiveData_ASII)")
        NotificationCenter.default.post(name: Notification.Name(rawValue: "standbyDevice_success"), object: nil)
        //刷新数据
        current_isStandByStatuse = will_isStandByStatuse
    }
