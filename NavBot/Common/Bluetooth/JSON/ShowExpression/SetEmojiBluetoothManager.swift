...
class ShowExpressionBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    var current_Emoji_paramDict = [String: Any]()

    //MARK: 1.初始化-单例
    static let shared = ShowExpressionBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String, paramDict: [String: Any]){
        fromVCType = type
        current_Emoji_paramDict = paramDict
        ...
        do{
            ...
            print("转化数据--失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "ShowExpression_fail"), object: nil)
            return
        }
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "ShowExpression"){
            //print("getDeviceInfo--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "ShowExpression_fail"), object: nil)
        }else{
            //展示表情不会返回数据，直接在1s后，默认成功
            DispatchQueue.main.asyncAfter(deadline: .now()+1.0, execute: {
                if !self.isHaveRecievedData{
                    print("ShowExpression_step--1秒内设备没有返回信息，直接认为操作成功了")
                    BluetoothManager.shared.isReceiveData = "notRecieved"
                    BluetoothManager.shared.recieveData_type = ""
                    NotificationCenter.default.post(name: Notification.Name(rawValue: "ShowExpression_success"), object: nil)
                }
            })
        }
    }
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetEmoji"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("ShowExpression--收到的数据:\(allReceiveData_ASII)")
