...
class SetWifiBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""

    //MARK: 1.初始化-单例
    static let shared = SetWifiBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String, paramDict: [String: Any]){
        ...
        do{
            ...
            print("转化数据--失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetWifi_fail"), object: nil)
        }
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetWifi"){
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("SetWifi_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetWifi"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("SetWifi--收到的数据:\(allReceiveData_ASII)")
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: "SetWifi_success"), object: nil)
    }
