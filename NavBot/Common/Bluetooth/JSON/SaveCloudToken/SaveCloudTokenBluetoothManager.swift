...
class SaveCloudTokenBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
   
    //MARK: 1.初始化-单例
    static let shared = SaveCloudTokenBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String, paramDict: [String: Any]){
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SaveCloudToken"){
            //print("getDeviceInfo--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SaveCloudToken_fail"), object: nil)
        }else{
            //现在等待返回数据--如果10秒内没有返回数据，就认为失败了
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("SaveCloudToken_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SaveCloudToken"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("SaveCloudToken--收到的数据:\(allReceiveData_ASII)")
        ...
        if let dict = try? JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] {
            print("解析成字典:", dict)
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SaveCloudToken_success"), object: nil)
            return
            //return
        }else {
            print("无法解析成字典")
