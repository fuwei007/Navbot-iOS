...
class GetDeviceInfoBluetoothManager: NSObject {
    ...
    //MARK: 1.初始化-单例
    static let shared = GetDeviceInfoBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "getDeviceInfo"){
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("getDeviceInfo_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "getDeviceInfo"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("getDeviceInfo--收到的数据:\(allReceiveData_ASII)")
