...
class OnServoBluetoothManager: NSObject {
    ...
    //MARK: 1.初始化-单例
    static let shared = OnServoBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        let param_dict: [String: Any] = ["type": "on_servo"]
        do{
            ...
            print("转化数据--失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "OnServo_fail"), object: nil)
        }
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "OnServ"){
            //print("getDeviceInfo--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "OnServ_fail"), object: nil)
        }else{
            //现在等待返回数据--如果10秒内没有返回数据，就认为失败了
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("OnServ_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "OnServ"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("OnServ--收到的数据:\(allReceiveData_ASII)")
        ...
        //print("OnServ--收到的数据2:\(allReceiveData_ASII2)")
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: "OnServ_success"), object: nil)
    }
