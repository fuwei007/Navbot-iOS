...
class SetDeviceNameBluetoothManager: NSObject {
    ...
    //MARK: 1.初始化-单例
    static let shared = SetDeviceNameBluetoothManager()
    ...
    //MARK: 2.包装数据并发送数据
    func sendBluetoothDataWith(type: String, send_param_dict: [String: Any]){
        fromVCType = type
        
        //1.参数数据转为[UInt8] (byte array)
        var param_command_bytes = [UInt8]()
        let param_dict = send_param_dict
        do{
            ...
            print("转化数据--失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetDeviceName_fail"), object: nil)
        }
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetDeviceName"){
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("SetDeviceName_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetDeviceName"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("SetDeviceName--收到的数据:\(allReceiveData_ASII)")
        
        
        var delete_send_comand_Bytes = allReceiveData_bytes
        ...
        if let dict = try? JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] {
            print("解析成字典:", dict)
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetDeviceName_success"), object: nil)
        }else {
            print("无法解析成字典")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetDeviceName_fail"), object: nil)
            return
