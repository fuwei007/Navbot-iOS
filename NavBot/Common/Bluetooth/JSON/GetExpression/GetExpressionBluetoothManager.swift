...
class GetExpressionBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    var current_Emoji_files = [String]()
    var current_device_name = ""
    var origin_send_command_number = 0

    //MARK: 1.初始化-单例
    static let shared = GetExpressionBluetoothManager()
    ...
    //MARK: 2.包装数据并发送数据
    func sendBluetoothDataWith(type: String, paramDict: [String: Any]){
        
        fromVCType = type
        
        //如果已经获取过表情文件，就直接成功：
        if current_device_name == BluetoothManager.shared.current_connecting_CBPeripheral?.name ?? "",
           current_Emoji_files.count > 0{
            NotificationCenter.default.post(name: Notification.Name(rawValue: "GetExpression_success"), object: nil)
            return
        }
        
        //1.参数数据转为[UInt8] (byte array)
        var param_command_bytes = [UInt8]()
        let param_dict = paramDict
        do{
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "GetExpression"){
            //print("getDeviceInfo--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "GetExpression_fail"), object: nil)
        }else{
            //现在等待返回数据--如果10秒内没有返回数据，就认为失败了
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    print("GetExpression_step--10秒内设备没有返回信息，直接认为操作失败了")
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "GetExpression"{
            return
        }
        BluetoothManager.shared.isReceiveData = "notRecieved"
        BluetoothManager.shared.recieveData_type = ""
        ...
        print("GetExpression--收到的数据:\(allReceiveData_ASII)")
        ...
        if let dict = try? JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] {
            print("解析成字典:", dict)
            if let emoji_files = dict["expression"] as? [String]{
                current_Emoji_files = emoji_files
                current_device_name = BluetoothManager.shared.current_connecting_CBPeripheral?.name ?? ""
                NotificationCenter.default.post(name: Notification.Name(rawValue: "GetExpression_success"), object: nil)
            }else{
                NotificationCenter.default.post(name: Notification.Name(rawValue: "GetExpression_fail"), object: nil)
            }
