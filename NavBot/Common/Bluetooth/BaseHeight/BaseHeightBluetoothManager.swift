...
class BaseHeightBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    
    var baseHeight_current: Int32 = 32
    var baseHeight_willChanged: Int32 = 32
    
    //MARK: 1.初始化-单例
    static let shared = BaseHeightBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        command_bytes.append(UInt8(baseHeight_willChanged))
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetupBaseHeight"){
            //print("SetTimePudcam--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupBaseHeight_fail"), object: nil)
        }else{
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+10.0, execute: {
                if !self.isHaveRecievedData{
                    ...
                    NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupBaseHeigh_fail"), object: nil)
                }
            })
        }
    }
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetupBaseHeight"{
            return
        }
        //结束此次指令
        BluetoothManager.shared.isReceiveData = "notRecieved"
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupBaseHeigh_success"), object: nil)
        //刷新数据
        baseHeight_current = baseHeight_willChanged
    }
}
