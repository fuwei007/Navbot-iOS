...
class AngularVelBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    
    var current_Angular_Vel = 50
    var will_Angular_Vel = 50
    
    //MARK: 1.初始化-单例
    static let shared = AngularVelBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        command_bytes.append(UInt8(bitPattern: Int8(will_Angular_Vel)))
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetupAngularVel"){
            //print("SetTimePudcam--\(type)--发送命令失败")
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupAngularVel_fail"), object: nil)
        }else{
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetupAngularVel"{
            return
        }
        //结束此次指令
        BluetoothManager.shared.isReceiveData = "notRecieved"
        ...
        print("SetupAngularVel--收到的数据:\(allReceiveData_ASII)")
        //目前只要接收到的数据和发送的数据是一样的，就认为成功了
        NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupAngularVel_success"), object: nil)
        //刷新数据
        current_Angular_Vel = will_Angular_Vel
    }
}
