...
class RollBluetoothManager: NSObject {
    ...
    //MARK: 1.初始化-单例
    static let shared = RollBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        command_bytes.append(UInt8(bitPattern: Int8(will_roll)))
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetupRoll"){
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupRoll_fail"), object: nil)
        }else{
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetupRoll"{
            return
        }
        //结束此次指令
        BluetoothManager.shared.isReceiveData = "notRecieved"
        ...
        print("SetupRoll--收到的数据:\(allReceiveData_ASII)")
        //目前只要接收到的数据和发送的数据是一样的，就认为成功了
        NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupRoll_success"), object: nil)
        //刷新数据
        current_roll = will_roll
    }
}
