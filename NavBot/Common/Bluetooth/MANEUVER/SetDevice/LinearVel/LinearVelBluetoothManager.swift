...
class LinearVelBluetoothManager: NSObject {
    
    var isHaveRecievedData = false
    var fromVCType = ""
    
    var current_Linear_Vel = 100
    var will_Linear_Vel = 100
    
    //MARK: 1.初始化-单例
    static let shared = LinearVelBluetoothManager()
    ...
    func sendBluetoothDataWith(type: String){
        ...
        let (linear_H, linear_L) = encodeLinear(Int16(will_Linear_Vel))
        ...
        if !BluetoothManager.shared.writeDataToDevice(writeData: command_Data, dataType: "SetupLinearVel"){
            NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupLinearVel_fail"), object: nil)
        }else{
    ...
    @objc func receivedAllDataFromPudcamDevice(){
        if BluetoothManager.shared.recieveData_type != "SetupLinearVel"{
            return
        }
        //结束此次指令
        BluetoothManager.shared.isReceiveData = "notRecieved"
        ...
        print("SetupLinearVel--收到的数据:\(allReceiveData_ASII)")
        //目前只要接收到的数据和发送的数据是一样的，就认为成功了
        NotificationCenter.default.post(name: Notification.Name(rawValue: "SetupLinearVel_success"), object: nil)
        //刷新数据
        current_Linear_Vel = will_Linear_Vel
    }
}
